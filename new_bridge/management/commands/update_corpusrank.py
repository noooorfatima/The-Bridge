...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if lang == "Greek" or lang == "greek":
             query = ("SELECT title, count(title) FROM new_bridge_wordappearencesgreek a JOIN `new_bridge_wordpropertygreek`p on\
 a.word_id = p.`id` GROUP by title ORDER BY count(title) DESC")
        elif lang == "Latin" or lang == "latin":
            query = ("SELECT title, count(title) FROM new_bridge_wordappearenceslatin a JOIN `new_bridge_wordpropertylatin`p on\
 a.word_id = p.`id` GROUP by title ORDER BY count(title) DESC")
        assert(query)
