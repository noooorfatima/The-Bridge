...
import networkx
from networkx import Graph, NetworkXError, read_gpickle, write_gpickle, bfs_tree, to_dict_of_dicts
...
def lemmataEquivalenciesInWorkbook(workbook):
    ...
    for row in workbook.active:
        if not formats:
            ...
            column_combinations = list(combinations(list(range(len(row))), 2))
            continue
